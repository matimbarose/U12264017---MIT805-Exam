...
class director_name(MRJob):
    def steps(self):
        return[
            MRStep(mapper=self.mapper_get_director_name,
                  reducer=self.reducer_count_director_name)
        ]
#Mapper function 
    def mapper_get_director_name(self, _, line):
       ...
       for row in reader:
           ...
           #yield director_name, 1
           #yield director_name, pd.to_numeric(Fine_amount)
#Reducer function
    def reducer_count_director_name(self, key, values):
